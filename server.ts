...
>>>>>>> 550ff2d (Build universal self-completing task system)
}
...
- For "make it faster/slower", use SPEED with a numeric speed value.
      - SPEED MUST always be between 0.25 and 4.0 inclusive. NEVER generate a SPEED below 0.25 or above 4.0.
- SPEED MUST always be between 0.25 and 4.0 inclusive. NEVER generate a SPEED below 0.25 or above 4.0.
...
- If the user asks for a general style change such as cinematic, create
  concrete operations such as FILTER, COLOR-related fields, SPEED, TEXT,
  AUDIO, or TRANSITION when appropriate rather than only describing the idea.
...
`;
    ...
    return res.json({
      ...
      data: {
        ...
        explanation: data.explanation || "AI generated an editing plan.",
...
app.post("/api/ai/script", async (req, res) => {
  try {
    ...
    const vibe = String(req.body?.vibe || "Viral TikTok / Reel");
...
`;
    ...
    try {
      ...
      console.error("Gemini script returned invalid JSON:", text);
      return res.status(502).json({
        success: false,
    ...
    return res.json({
      ...
      data: {
        ...
        estimatedDuration: Number(data.estimatedDuration || durationSec),
        viralScore: Number(data.viralScore || 0),
        targetAspect: String(data.targetAspect || aspectRatio),
        backgroundMusicVibe: String(data.backgroundMusicVibe || vibe),
        scenes: data.scenes,
        suggestedHashtags: Array.isArray(data.suggestedHashtags)
...
// VIDOAI_UNIVERSAL_SYSTEM_ROUTE
app.post("/api/universal/plan", async (req, res) => {
  try {
    const { goal = "", input = {}, priority = "normal" } = req.body ?? {};
